...
# Basic parameters remain similar.
PARAMS = {
    Product.CROISSANTS: {
        "fair_value": 100,
        "take_width": 1,       # Default take width (will be scaled dynamically)
        "clear_width": 0.5,
        "volume_limit": 250,
    },
    Product.JAMS: {
    ...
    # Basket parameters – these still use spread and conversion metrics.
    Product.PICNIC_BASKET1: {
        ...
        "zscore_threshold": 1.5,  # Adjusted for sensitivity with dynamic spread stats.
        "target_position": 20,
    },
...
class Trader:
    def __init__(self, params=None):
        ...
        # Price history for dynamic fair value estimation for individual products.
        # Stored as a dictionary mapping product -> list of recent midprices.
        self.price_history = { 
            Product.CROISSANTS: [],
            Product.JAMS: [],
            Product.DJEMBES: []
        }
        # Length of the history window (can be tuned).
        self.history_window = 20

    ####################################################
    # Helper function: dynamic fair value estimation.
    ####################################################
    def update_dynamic_fair_value(self, product: str, order_depth: OrderDepth) -> (float, float):
        """
        Calculate the midprice from the order depth and update the price history.
        Returns a tuple: (dynamic_fair_value, dynamic_take_width)
        where dynamic_take_width is scaled by the volatility over the history window.
        """
        if order_depth.buy_orders and order_depth.sell_orders:
            ...
            midprice = self.params[product]["fair_value"]
        # Append and maintain the history list.
        hist = self.price_history[product]
        hist.append(midprice)
        if len(hist) > self.history_window:
            hist.pop(0)
        # Compute dynamic fair value as the average.
        dynamic_fair_value = np.mean(hist)
        # Dynamic volatility as standard deviation.
        volatility = np.std(hist)
        # For take_width, we use max(default, factor * volatility)
        default_tw = self.params[product]["take_width"]
        dynamic_take_width = max(default_tw, 1.5 * volatility)
        return dynamic_fair_value, dynamic_take_width
    ...
    def spread_orders(self, basket_type: str, order_depths: Dict[str, OrderDepth], basket_position: int, spread_history: List[float]):
        ...
        # Overpriced basket: sell basket and buy its components.
        if zscore >= threshold:
            ...
            return component_orders
        # Underpriced basket: buy basket and sell its components.
        if zscore <= -threshold:
            target_quantity = abs(self.params[basket_type]["target_position"] - basket_position)
            if target_quantity <= 0:
                return None
    ...
    def run(self, state: TradingState):
        ...
        # Trade individual products using dynamic fair value.
        for product in [Product.CROISSANTS, Product.JAMS, Product.DJEMBES]:
            if product in state.order_depths:
                # Compute dynamic fair value and adjusted take_width.
                dynamic_fv, dynamic_tw = self.update_dynamic_fair_value(product, state.order_depths[product])
                position = state.position[product] if product in state.position else 0
                orders_take, buy_vol, sell_vol = self.take_orders(product, state.order_depths[product], dynamic_fv, dynamic_tw, position)
                clear_width = self.params[product]["clear_width"]
                orders_clear, buy_vol, sell_vol = self.clear_orders(product, state.order_depths[product], dynamic_fv, clear_width, position, buy_vol, sell_vol)
                orders_mm = []
                # Market making with dynamic parameters.
                self.market_make(product, orders_mm, dynamic_fv - dynamic_tw, dynamic_fv + dynamic_tw, position, buy_vol, sell_vol)
